...
function auth(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    if (err) return res.status(403).json({ message: 'Sessão expirada. Faça login novamente.' });
    req.user = user;
    next();
  });
}
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const [r] = await pool.execute('INSERT INTO usuarios (nome, email, senha) VALUES (?, ?, ?)', [nome.trim(), email.toLowerCase().trim(), hash]);
    res.status(201).json({ message: 'Usuário criado com sucesso', userId: r.insertId });
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') return res.status(400).json({ message: 'Este e-mail já está cadastrado' });
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const user = rows[0];
    if (!user || !(await bcrypt.compare(senha, user.senha))) return res.status(401).json({ message: 'E-mail ou senha inválidos' });
...
app.get('/api/hoteis/:id/disponibilidade', async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT checkin, checkout FROM reservas WHERE hotel_id = ?', [req.params.id]);
    res.json(rows);
  } catch (err) {
...
app.get('/api/reservas', auth, async (req, res) => {
  try {
    const [rows] = await pool.execute(`
      SELECT r.id, r.hotel_id, r.checkin, r.checkout,
    ...
    console.error('Erro GET /reservas:', err.message);
...
app.post('/api/reservas', auth, async (req, res) => {
  try {
    ...
    if (!hotel_id || !checkin || !checkout) return res.status(400).json({ message: 'hotel_id, checkin e checkout são obrigatórios' });
    if (checkout <= checkin) return res.status(400).json({ message: 'Check-out deve ser depois do check-in' });
    const [hotel] = await pool.execute('SELECT id FROM hoteis WHERE id = ?', [hotel_id]);
    if (!hotel.length) return res.status(404).json({ message: 'Hotel não encontrado' });
    ...
    if (conflito.length) return res.status(400).json({ message: 'Hotel já reservado para esse período' });
    const [result] = await pool.execute(
      'INSERT INTO reservas (usuario_id, hotel_id, checkin, checkout) VALUES (?, ?, ?, ?)',
      [req.user.userId, hotel_id, checkin, checkout]
    );
    res.status(201).json({ message: 'Reserva criada com sucesso', reservaId: result.insertId });
  } catch (err) {
    console.error('Erro POST /reservas:', err.sqlMessage || err.message);
    res.status(500).json({ message: `Erro ao criar reserva: ${err.sqlMessage || err.message}` });
  }
});

app.delete('/api/reservas/:id', auth, async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT id FROM reservas WHERE id = ? AND usuario_id = ?', [req.params.id, req.user.userId]);
